...
class FMCIBScoreCorrector:
    def __init__(self, 
        ...
        self.classifier_scale = torch.nn.Parameter(torch.ones(1) * classifier_scale, requires_grad=True)
    ...
    def modify_score(self, ddim_sampler: DDIMSampler, score, x, t, c, index, **kw):
        ...
        with torch.enable_grad():
            ...
            if self.use_optimizer_on_classifier_scale: 
                # 1 MCS
                x_prev, pred_x0 = ddim_sampler.ddim_solver._retrieve_xprev(x, modified_score)
                log_probs2 = torch.log_softmax(self.measurement.measure(x_prev, c, max(0, t - 1) / ddim_sampler.ddpm_num_timesteps), dim=-1)
                y_log_probs2 = log_probs2[range(len(log_probs2)), y.view(-1)]
                grad2 = torch.autograd.grad((y_log_probs2 - y_log_probs).sum(), self.classifier_scale)[0]
                # update classifier scale
                grad2 = grad2 + self.weight_decay * self.classifier_scale
                ...
                self.classifier_scale = self.classifier_scale - self.lr * m_hat_t / (v_hat_t.sqrt() + 1e-8)
