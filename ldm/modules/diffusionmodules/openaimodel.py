...
class ControlNetUNetModel(UNetModel):
    def __init__(
        ...
        self.control_input_map = TimestepEmbedSequential(
            conv_nd(self.dims, 1, 16, 3, padding=1),
            nn.SiLU(),
            conv_nd(self.dims, 16, 32, 3, padding=1),
            nn.SiLU(),
            zero_module(conv_nd(self.dims, 32, self.in_channels, 3, padding=1))
        )
        self.control_net = copy.deepcopy(self.input_blocks)
        
        ch = self.input_block_chans
        self.control_input_zero_convs = nn.ModuleList()
        self.control_middle_zero_conv = zero_module(conv_nd(self.dims, ch[-1], ch[-1], 3, padding=1))
        for i in range(len(self.control_net)):
            self.control_input_zero_convs.append(zero_module(conv_nd(self.dims, ch[i], ch[i], 3, padding=1)))
                
        # self.freeze_control_net()
        # self.unfreeze_unet()
    
    def copy_params(self):
        unet_params = dict(self.input_blocks.named_parameters())
    ...
    def forward(self, inputs, timesteps=None, context=None, y=None, **kwargs):
        ...
        zx = self.control_input_map(ctrl, emb, context) + x
        ...
        h = self.middle_block(h + self.control_middle_zero_conv(zh), emb, context)
        for module in self.output_blocks:
