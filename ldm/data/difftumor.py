...
def reorient_func(img, targ_axcode="RAS"):
    ...
    targ_ornt = nibabel.orientations.axcodes2ornt(targ_axcode)
    if (ori_ornt == targ_ornt).all():
        return img, ori_ornt
    else:
        # print("The image is not in the target orientation. Reorienting...")
        transform = nibabel.orientations.ornt_transform(ori_ornt, targ_ornt)
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.trainData = TorchioDataloader(
        ...
        self.validData = TorchioDataloader(
...
class TorchioDataloader(Dataset):
    def __init__(self, iter_num=0, batch_size=0, class_num=3, num=None, transform=None, random_sample=False, 
                 ...
                 normalize=False, thresh_ls=[], norm_ls=[], respacing=False, crop_shape=None, target_spacing=1, dataset_name=None):
        """
        Args:
        ...
        if dataset_name is not None:
            ...
            print(f"using {os.path.join(base, dataset_path_file)} as base data folder")
            image_list = os.path.join(base, dataset_path_file, "image.txt")
            coarseg_list = os.path.join(base, dataset_path_file, "totalseg.txt")
            label_list = os.path.join(base, dataset_path_file, "label.txt")
        ...
        if max_mask_num is None:
            if seg_list:
                ...
                self.transform = transforms.Compose([
                    ...
                    MultiThreshNormalized(include=['image'], thresh_ls=[[-1000, -200, 200, 1000]], norm_ls=[[0,0.2,0.8,1]]),
                    ToTensor(concat_key=[]),
                    RemapAndExpandLabels(remapping={2: 1, 1: 0})
                ])
        else: 
...
class RemapAndExpandLabels(object):
    def __init__(self, remapping=None):
        self.remapping = remapping
    def __call__(self, sample):
        old_label = sample['label']
        new_label = torch.zeros_like(old_label)
        for k in old_label.unique():
            if int(k) not in self.remapping:
                new_label[old_label == k] = k
            else:
                new_label[old_label == k] = self.remapping[int(k)]
        sample['label'] = new_label[None]
        return sample
