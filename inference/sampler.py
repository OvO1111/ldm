...
from ldm.models.diffusion.ddpm import LatentDiffusion, DDPM, Img2MaskDiffusion
...
class InferLatentDiffusion(LatentDiffusion):
    def __init__(self, test_scheme, eval_scheme, **diffusion_kwargs):
        super().__init__(**diffusion_kwargs)
        if test_scheme is None:
            self.scheme = "normal"
        else: self.scheme = test_scheme
    ...
    def test_step(self, batch, batch_idx):
        ...
        return loss_dict_ema, loss_dict_no_ema
        
    def test_step_end(self, output_results):
        loss_dict_ema, loss_dict_no_ema = output_results
    ...
    @torch.no_grad()
    def log_images(self, batch, log_metrics=False, log_grouped=False, *args, **kwargs):
        if self.scheme == "normal":
            ...
            if self.scheme == "mixup":
                assert b > 1
                alpha = kwargs.get("mixup_ratio", .2)
                ...
                if sample:
                    ...
                    for i in range(b - 1):
                        ...
                        mask_local_mix.append(mask_fine_reshaped[i] * (1 - alpha) + mask_coarse[*z_coarse_masked_cropped[i]] * alpha)
        ...
        if self.eval_scheme is not None and len(self.eval_scheme) > 0 and log_metrics:
            metrics = self.log_eval(x_samples, x, log_grouped)
            # print(metrics)
    ...
    @torch.no_grad()
    def log_eval(self, x, y, log_grouped=False):
        metrics = dict()
        b, c, *shp = x.shape
